        ...
        <div class="content">
            <transition :name="slideDirection === 'forward' ? 'slide-horizontal' : 'slide-horizontal-reverse'"
                            ...
                            <div class="scroll">
                                <div v-for="product in products" :key="product.id" class="product-row">
                                    ...
                                    <div class="col col-actions">
                                        <button class="edit-btn" @click="editProduct(product.id)"><img
                                                src="/admin/pencil.png" alt=""></button>
                                        <button class="delete-btn" @click="deleteProduct(product.id)"><img
                                                src="/admin/trash.png" alt=""></button>
                                    </div>
                                </div>
                            </div>
                ...
                <div v-else key="edit" class="slide-content edit-content">
                    <transition :name="slideDirection === 'forward' ? 'slide-horizontal' : 'slide-horizontal-reverse'"
                        mode="out-in">
                        <div v-if="selectedProduct" key="edit-detail" class="edit-detail">
                            <AdminProduct :product="selectedProduct" @back="goBackToList" @saved="onProductSaved" />
                        </div>
                        <div v-else key="edit-error" style="color:red; padding:2em;">Erreur : produit introuvable (id={{
                            editingProductId }})<br>Produits chargés : {{ products.length }}</div>
                    </transition>
                </div>
            </transition>
        </div>
